...
public class SimpleTextService implements TextService {
    ...
    public String toJumpCase(String text) {
        char[] symbolArr = text.toCharArray();
        for (int i = 0; i < symbolArr.length; i++) {
            if (i % 2 == 0) {
                symbolArr[i] = Character.toLowerCase(symbolArr[i]);
            } else symbolArr[i] = Character.toUpperCase(symbolArr[i]);
        }
        String jumpString = String.valueOf(symbolArr);
        return jumpString; //TODO
    }
    ...
    public boolean isPalindrome(String string) {
        ...
        return revers.equalsIgnoreCase(string); //TODO
    }
}
